...
class HierarchicalIndexer:
    def __init__(self, llm, vector_store: Chroma, summary_tree_path: str,  chunk_size: int = 500, chunk_overlap: int = 128):
        ...
        self.status_file_path = Path(summary_tree_path) / "status.json" 
        ...
        async with self._index_lock:
            ...
            for root_str in folders_to_index:
                ...
                for doc in tqdm(raw_docs, desc=f"{root_str}"):
                    ...
                    # set doc source to absolute path for consistency
                    doc.metadata["source"] = file_path_str
                    active_files.add(file_path_str)

                    # file already exists & is unchanged
                    if file_path_str in status_file and content_hash == status_file[file_path_str]["hash"]:
                        summary_result = status_file[file_path_str]["summary"]
                    else:
                        was_updated = True
                        num_modified_files += 1 
                        # file already exists & was changed
                        if file_path_str in status_file:
                        ...
                        # update cache
                        status_file[file_path_str] = {"hash": content_hash, "summary": summary_result}
            ...
            removed_paths = [path for path in status_file.keys() if path not in active_files] 
            if removed_paths:
                was_updated = True
            ...
            if was_updated:
                ...
                with open(self.status_file_path, "w", encoding="utf-8") as f:
                    json.dump(status_file, f, indent=2, ensure_ascii=False)
                print(f"[Indexer] Status file saved to {self.status_file_path}")
                if new_chunked_docs:
