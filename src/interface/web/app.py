...
class IndexRequest(BaseModel):
    folder_path: str
async def stream_agent_progress(prompt: str, mode: str):
    """
    ...
    """
    yield json.dumps({"type": "init", "mode": mode}) + "\n"
    ...
    if not state.values.get("messages"):
        if mode == "task":
            ...
            sys_msg = "You are a helpful assistant. Answer questions based on your indexed knowledge and context."
...
async def chat_endpoint(req: ChatRequest):
    return StreamingResponse(
        stream_agent_progress(req.prompt, req.mode), 
